...
always feasible ($c=0$) and bounded (the box on $c_i$). Value $0$ means $p\in\mathrm{cone}(E)$ by Farkas, so $p$ is redundant regardless of how incomplete $E$ still is. A positive value says nothing about $p$ itself; it just says $E$ is missing an extremal ray. The optimizer $c$ then points at one: the tie-broken maximizer of $c\cdot r_i$ over the remaining candidates is a vertex, joins $E$, and $p$ gets retested. In exact arithmetic that is a proof; a tie broken in floating point can admit a redundant ray, which the cleanup pass re-checks. The main sweep then costs $n+|E|$ LPs, each with at most $|E|$ rows, on one persistent warm-started HiGHS model.
